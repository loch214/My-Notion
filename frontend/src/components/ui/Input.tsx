  ...
  ({ className, error, type = 'text', ...props }, ref) => {
    return (
      <input
        type={type}
        className={cn(
          'flex h-11 w-full rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface-low)] px-4 py-2.5 text-sm text-[color:var(--text)] transition-all placeholder:text-[color:var(--muted)] hover:border-[color:var(--border-focus)]/30 focus:border-[color:var(--border-focus)] focus:ring-2 focus:ring-[color:var(--accent)]/15 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50',
...
export const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ className, error, rows = 4, ...props }, ref) => {
    return (
      <textarea
        className={cn(
          'flex min-h-[96px] w-full rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface-low)] px-4 py-3 text-sm text-[color:var(--text)] transition-all placeholder:text-[color:var(--muted)] hover:border-[color:var(--border-focus)]/30 focus:border-[color:var(--border-focus)] focus:ring-2 focus:ring-[color:var(--accent)]/15 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 resize-y',
          error ? 'border-rose-500 focus:ring-rose-500/20 focus:border-rose-500' : '',
          className
        )}
        ref={ref}
